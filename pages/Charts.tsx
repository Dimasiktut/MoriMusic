    ...
    <div className="p-5 pb-32">
      ...
      <div className="flex border-b border-white/5 mb-6">
        <button
          ...
          className={`flex-1 pb-3 text-[10px] font-black uppercase tracking-widest transition-colors relative ${chartType === 'week' ? 'text-sky-400' : 'text-zinc-600'}`}
        >
          {t('charts_week')}
        ...
        <button
          ...
          className={`flex-1 pb-3 text-[10px] font-black uppercase tracking-widest transition-colors relative ${chartType === 'month' ? 'text-sky-400' : 'text-zinc-600'}`}
        >
          {t('charts_month')}
      ...
      <div className="space-y-2.5">
        {loading ? (
            ...
            <div 
                key={track.id} 
                className="flex items-center gap-3.5 bg-zinc-900/40 p-2.5 rounded-[1.25rem] active:bg-zinc-800 transition-all cursor-pointer border border-white/5 hover:border-sky-500/30 group"
                onClick={() => onPlayTrack(track)}
            >
                <div className={`w-6 text-center font-black text-lg italic ${index < 3 ? 'text-sky-400' : 'text-zinc-700'}`}>
                {index + 1}
                </div>
                ...
                <div className="flex-1 min-w-0">
                    <h4 className="text-white text-xs font-black truncate uppercase italic tracking-tight">{track.title}</h4>
                    <p className="text-zinc-500 text-[10px] font-bold truncate mt-0.5">
                        {track.uploaderName} • {track.plays.toLocaleString()}
                    </p>
                </div>

                <div className="w-8 h-8 rounded-full bg-white/5 flex items-center justify-center text-sky-400 group-hover:bg-sky-400 group-hover:text-black transition-all">
